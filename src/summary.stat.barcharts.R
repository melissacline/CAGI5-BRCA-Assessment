...
pp<- ggplot(as, aes(x=reorder(X, sens.Mean), y=sens.Mean, fill=X)) +
    geom_bar(stat="identity", color="black",   position=position_dodge()) +	
             coord_flip() + scale_fill_manual(values=as$color) + guides(fill=FALSE) +
...
pp<- ggplot(as, aes(x=reorder(X, spec.Mean), y=spec.Mean, fill=X)) +
    geom_bar(stat="identity", color="black",   position=position_dodge()) +
             coord_flip() + scale_fill_manual(values=as$color) + guides(fill=FALSE) +
...
pp<- ggplot(as, aes(x=reorder(X, pearson.coeff.Mean), y=pearson.coeff.Mean, 
            fill=X)) +
    geom_bar(stat="identity", color="black",   position=position_dodge()) +
             coord_flip() + scale_fill_manual(values=as$color) + guides(fill=FALSE) +
